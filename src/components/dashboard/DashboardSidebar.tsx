  ...
  return (
    <aside className="flex h-full w-48 flex-col border-r border-white/[0.06] bg-[#0d0d0d]">
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto px-2.5 py-4">
        <CollapsibleGroup label="Account" icon={<UserCircle2 className="h-4 w-4 shrink-0" />}
          open={accountOpen || isAccountActive} onToggle={() => setAccountOpen(!accountOpen)} isActive={isAccountActive}>
          {accountSubItems.map((item) => {
            const Icon = item.icon;
            return <SubLink key={item.href} href={item.href} label={item.label} isActive={pathname === item.href}><Icon className="h-3.5 w-3.5 shrink-0" /></SubLink>;
          })}
        </CollapsibleGroup>
        {standaloneItems.map((item) => <StandaloneLink key={item.href} item={item} pathname={pathname} />)}
        <CollapsibleGroup label="Advance" icon={<Gem className="h-4 w-4 shrink-0" />}
          open={advanceOpen || isAdvanceActive} onToggle={() => setAdvanceOpen(!advanceOpen)} isActive={isAdvanceActive}>
      ...
      <div className="flex flex-col border-t border-white/[0.06]">
        <div className="flex flex-col gap-1.5 px-3 py-3">
          <a href="https://discord.gg/brazy" target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-2 text-xs text-white/30 transition-colors hover:text-white/60">
            <HelpCircle style={{ width: 13, height: 13 }} /> Help Center
          </a>
          ...
          <div className="flex items-center justify-between">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <div className="flex flex-col min-w-0">
                <span className="truncate text-xs font-medium text-white/70" style={{ maxWidth: 80 }}>{username ?? "..."}</span>
                <span className="truncate text-[10px] text-white/25" style={{ maxWidth: 80 }}>brazy.it/{username ?? "..."}</span>
              </div>
            </div>
            <div style={{ position: "relative" }}>
              <button onClick={() => setShowPopover(!showPopover)} className="text-white/30 transition-colors hover:text-white/60">
                <MoreHorizontal style={{ width: 15, height: 15 }} />
              </button>
              {showPopover && (
                <div ref={popoverRef} style={{ position: "absolute", bottom: "100%", right: 0, minWidth: 130, zIndex: 50, background: "#141414", border: "1px solid #1f1f1f", borderRadius: 12, padding: 6, display: "flex", flexDirection: "column", gap: 2 }}>
                  <Link href="/account/settings" onClick={() => setShowPopover(false)}
                    style={{ display: "block", fontSize: 12, padding: "7px 10px", borderRadius: 8, color: "#a5a4a4", textDecoration: "none" }}
                    onMouseEnter={e => { e.currentTarget.style.background = "rgba(255,255,255,0.05)"; }}
                    onMouseLeave={e => { e.currentTarget.style.background = "transparent"; }}>
                    Settings
                  </Link>
                  <button onClick={handleSignOut}
                    style={{ display: "block", width: "100%", textAlign: "left", fontSize: 12, padding: "7px 10px", borderRadius: 8, color: "#f87171", background: "none", border: "none", cursor: "pointer" }}
                    onMouseEnter={e => { e.currentTarget.style.background = "rgba(255,255,255,0.05)"; }}
                    onMouseLeave={e => { e.currentTarget.style.background = "transparent"; }}>
                    <div className="flex items-center gap-2"><LogOut style={{ width: 12, height: 12 }} /> Sign out</div>
                  </button>
                </div>
              )}
            </div>
...
function StandaloneLink({ item, pathname }: { item: { href: string; label: string; icon: React.ElementType }; pathname: string }) {
  ...
  const Icon = item.icon;
  return (
    <Link href={item.href} className={`flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium transition ${isActive ? "bg-white/[0.06] text-violet-300" : "text-white/40 hover:bg-white/[0.04] hover:text-white/70"}`}>
...
function CollapsibleGroup({ label, icon, open, onToggle, isActive, children }: { label: string; icon: React.ReactNode; open: boolean; onToggle: () => void; isActive: boolean; children: React.ReactNode }) {
  return (
    <div>
      <button onClick={onToggle} className={`flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium transition ${open || isActive ? "text-white/70" : "text-white/40 hover:bg-white/[0.04] hover:text-white/70"}`}>
      ...
      <div className={`overflow-hidden transition-all duration-200 ${open ? "max-h-60 opacity-100" : "max-h-0 opacity-0"}`}>
        <div className="ml-2 mt-0.5 space-y-0.5 border-l border-white/[0.06] pl-2">{children}</div>
      </div>
    </div>
  );
}
...
function SubLink({ href, label, isActive, children }: { href: string; label: string; isActive: boolean; children: React.ReactNode }) {
  return (
    <Link href={href} className={`flex items-center gap-2.5 rounded-lg px-3 py-1.5 text-sm font-medium transition ${isActive ? "bg-white/[0.06] text-violet-300" : "text-white/40 hover:bg-white/[0.04] hover:text-white/70"}`}>
      {children}{label}
    </Link>
  );
}
