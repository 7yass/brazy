...
import { Award, Plus, ShoppingCart, Upload, GripVertical, Trash2, Star } from "lucide-react";
    ...
    <div className="p-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-white/40">
            Create and manage custom badges to display on your profile.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <span className="rounded-xl bg-white/[0.04] px-3.5 py-1.5 text-xs text-white/40">
          ...
          <button className="flex items-center gap-2 rounded-xl border border-white/[0.08] px-4 py-2.5 text-sm font-medium text-white/50 transition hover:bg-white/[0.04] hover:text-white">
      ...
      {badges.length === 0 ? (
        ...
        <div className="space-y-2">
          {badges.map((badge, index) => (
            <div
              key={badge.id}
              className="group flex items-center gap-3 rounded-2xl border border-white/[0.06] bg-white/[0.02] px-4 py-3.5 transition hover:border-white/[0.12]"
            >
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl border border-white/[0.08] bg-[#0f0d1a] p-6 shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="color"
                    value={form.color || "#a855f7"}
                    onChange={(e) => setForm({ ...form, color: e.target.value })}
                    className="h-9 w-9 cursor-pointer rounded-xl border border-white/10 bg-transparent"
                  />
