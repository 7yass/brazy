  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-white">
            Discover the perfect template for your Profile
          </h1>
        </div>
        <p className="mt-2 text-sm text-white/40">
          Browse community-created templates, or design your own to share.
        </p>
      </div>
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <button className="flex items-center gap-2 rounded-xl border border-dashed border-white/[0.12] px-4 py-2 text-sm text-white/50 transition hover:border-violet-500/50 hover:text-violet-400">
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            ...
            className="w-full rounded-xl border border-white/[0.06] bg-white/[0.02] py-2 pl-9 pr-3.5 text-sm text-white outline-none transition focus:border-violet-400/50"
          />
        </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((template) => (
            <div
              key={template.id}
              className="group cursor-pointer rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden transition hover:border-white/[0.12] hover:bg-white/[0.04]"
            >
              <div className={`aspect-video bg-gradient-to-br ${template.gradient} relative`}>
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition bg-black/40">
                  <button className="flex items-center gap-2 rounded-xl bg-white px-4 py-2 text-sm font-medium text-black transition hover:bg-white/90">
              ...
              <div className="p-4">
                ...
                <div className="mt-3 flex items-center gap-3 text-xs text-white/30">
                  <span className="flex items-center gap-1">
                    <Heart className="h-3 w-3" /> {template.favorites}
                  </span>
                  <span className="flex items-center gap-1">
                    <Clock className="h-3 w-3" /> {template.uses} uses
                  </span>
                </div>
              </div>
            </div>
